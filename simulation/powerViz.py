#This bit of code compares the total energy with each node element
...
ax.plot(bdf['Run'],Etotal,label='Total')
ax.plot(bdf['Run'],bdf['Ecpu'],'-*', label='CPU')#plot (x,y)
ax.plot(bdf['Run'],bdf['Elpm'],'-x',label='LPM')
ax.plot(bdf['Run'],bdf['Etx'],'-', label='TX')
ax.plot(bdf['Run'],bdf['Erx'],'-D', label='RX')
